...
def train(model, train_loader, label_list, mode="layer-wise", epochs=args.epochs, device=args.device):
    ...
    for i in range(loop_size): # i refers to head or layer
        for epoch in tqdm(range(1, epochs + 1)):
            for idx, example_batched in enumerate(tqdm(train_loader)):
                optimizer.zero_grad()
    ...
    with torch.no_grad():
        for i in range(loop_size):  # i refers to head or layer
            ...
            for example_batched in tqdm(eval_loader):
                ...
                outputs = model(input_ids, attention_mask)
                logits = outputs[i][:, 0, :] # CLS
                preds = torch.argmax(logits, dim=-1).float().to(device)
                ...
                results = metric.add_batch(predictions=true_predictions, references=true_labels)
