...
def main():
    ...
    send_example_telemetry("run_paswx", model_args)
    ...
    if training_args.do_train:
        if model_args.train_language is None:
            ...
            if model_args.train_language == "all_languages":
                dfs = []
                for lang in DEFALT_LANGUAGES["pawsx"][:-1]:
                    df = load_dataset("paws-x", lang, split="train", cache_dir=model_args.cache_dir, use_auth_token=True if model_args.use_auth_token else None)
                    dfs.append(df)
